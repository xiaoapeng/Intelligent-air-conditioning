...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagState_Conditioning)
    {
        info->event[info->num] = EVENT_State_Conditioning;
        info->num++;
        dataPoints->valueState_Conditioning = gizStandardDecompressionValue(State_Conditioning_BYTEOFFSET,State_Conditioning_BITOFFSET,State_Conditioning_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagIntelligent_model)
    {
        info->event[info->num] = EVENT_Intelligent_model;
        info->num++;
        dataPoints->valueIntelligent_model = gizStandardDecompressionValue(Intelligent_model_BYTEOFFSET,Intelligent_model_BITOFFSET,Intelligent_model_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueState_Conditioning != cur->valueState_Conditioning)
    {
        GIZWITS_LOG("valueState_Conditioning Changed\n");
        ret = 1;
    }
    if(last->valueIntelligent_model != cur->valueIntelligent_model)
    {
        GIZWITS_LOG("valueIntelligent_model Changed\n");
        ret = 1;
    }
    ...
    if(last->valuebuzzing != cur->valuebuzzing)
    {
        GIZWITS_LOG("valuebuzzing Changed\n");
        ret = 1;
    }

    if(last->valueRoom_Temperature != cur->valueRoom_Temperature)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRoom_Temperature Changed\n");
            ret = 1;
        }
    }
    if(last->valueroom_humidity != cur->valueroom_humidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueroom_humidity Changed\n");
            ret = 1;
        }
    }
    if(last->valueillumination_intensity != cur->valueillumination_intensity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueillumination_intensity Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(State_Conditioning_BYTEOFFSET,State_Conditioning_BITOFFSET,State_Conditioning_LEN,(uint8_t *)devStatusPtr,dataPoints->valueState_Conditioning);
    gizStandardCompressValue(Intelligent_model_BYTEOFFSET,Intelligent_model_BITOFFSET,Intelligent_model_LEN,(uint8_t *)devStatusPtr,dataPoints->valueIntelligent_model);
    gizStandardCompressValue(buzzing_BYTEOFFSET,buzzing_BITOFFSET,buzzing_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebuzzing);
    ...
    devStatusPtr->valueRoom_Temperature = gizY2X(Room_Temperature_RATIO,  Room_Temperature_ADDITION, dataPoints->valueRoom_Temperature); 
    devStatusPtr->valueroom_humidity = gizY2X(room_humidity_RATIO,  room_humidity_ADDITION, dataPoints->valueroom_humidity); 
    devStatusPtr->valueillumination_intensity = gizY2X(illumination_intensity_RATIO,  illumination_intensity_ADDITION, dataPoints->valueillumination_intensity); 
